  ...
  for (int i = 1; i < argc; i++) {
    ...
    for (j = 1; argv[i][j] != '\0'; j++) {
      if (argv[i][j] == 'V') {
        ...
        op_use_refine_list = 1;
        // set the element groth ratio (in adaptive mesh refinement).
        if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            (argv[i][j + 1] == '.')) {
          k = 0;
          while (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
          ...
          workstring[k] = '\0';
          op_elem_growth_ratio = (REAL) strtod(workstring, (char **) NULL);
        }
      } else if (argv[i][j] == 'm') {
        // Check if a scale factor is provided (-m#)
        j++;
        if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            (argv[i][j + 1] == '.')) {
          k = 0;
          ...
          workstring[k] = '\0';
          op_metric_scale = atof(workstring);
        }
        // Check if a background mesh is provided (after -m filename_
        if (argc >= (i+2)) {
          if (argv[i+1][0] != '-') {
        ...
        if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
          ...
          workstring[k] = '\0';
          op_tol_rel_gap = atof(workstring);
        }
        if ((argv[i][j + 1] >= ',') || (argv[i][j + 1] >= '/')) {
          j++;
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            REAL tol_minangle = atof(workstring); // degree.
            if (tol_minangle > 0.) {
              op_tol_minangle = tol_minangle * PI / 180.;
        ...
        if (argv[i][j+1] == 'N') { // -IN
          ...
          io_firstindex = 1; j++;
        } else if (argv[i][j+1] == 'M') { // -IM
          io_no_out_triang = 1; j++;
        ...
        } else if (argv[i][j+1] == 's') { // -Is
          io_save_sol_to_vtk = 1; j++;
        } else if (argv[i][j+1] == 'm') { // -Im
          io_save_inria_mesh = 1; j++;
        }
        // End of '-I'
      }
    } // for j
...
int Triangulation::read_nodes()
{
  ...
  for (i = 0; i < pnum; i++) {
    ...
    //if (feof(infile)) break; // This line caused an error by debug_stefan.node
  ...
  if (infile != NULL) {
    ...
    if (mtrnum == ct_in_vrts) {
      ...
      for (i = 0; i < mtrnum; i++) {
        ...
        if (mtrsize == 1) {
          in_vrts[i].val = atof(pstr) * op_metric_scale;
        } else if (mtrsize == 3) {
...
void Triangulation::save_inria_mesh()
{
  ...
  if (tr_steiners != NULL) {
    for (i = 0; i < tr_steiners->used_items; i++) {
      ...
      if (vrt->is_deleted()) continue;
      int tag = 0;
      if (vrt->on_seg.tri != NULL) {
        tag = vrt->on_seg.tri->tag;
      }
...
void Triangulation::save_to_m()
{
  char filename[256];
  sprintf(filename, "%s.m", io_outfilename);
  FILE *outfile = fopen(filename, "w");
  ...
  int i, idx=1; // m index starts from 1.
  for (i = 0; i < ct_in_vrts; i++) {
    if (!io_keep_unused) { // no -IJ
      if (in_vrts[i].typ == UNUSEDVERTEX) continue;
    }
    fprintf(outfile, "Vertex %d %g %g 0\n",
            idx, in_vrts[i].crd[0], in_vrts[i].crd[1]);
    in_vrts[i].idx = idx;
    idx++;
  }
