...
// QUIT CODE
#define QUIT_ON_BUG         2
#define QUIT_ON_TODO        3
#define QUIT_ON_INPUT_ERR   4
...
class Vertex
{
 public:
  ...
  TriEdge    on_seg; // A boundary segment containing this vertex.
  TriEdge    on_omt; // A background domain triangle containing this vertex.
  ...
  void init() {
    ...
    idx = tag = typ = flags = 0;
    adj.tri = on_seg.tri = on_omt.tri = NULL;
    adj.ver = on_seg.ver = on_omt.ver = 0;
    Pair = Next = Prev = NULL;
    Data = NULL;
  }
...
class Triangulation
{
 public:
  ...
  bool tr_free_domain;  // shall we delete OMT_domain in the destructor?
  bool tr_free_voronoi; // shall we delete Voronoi in the destructor?
  ...
  int  io_no_out_triang; // 0, -IM Do not output triangulation (.1.ele)
  ...
  int  io_save_sol_to_vtk; // 0, -Is
  int  io_save_inria_mesh; // 0, -Im
  ...
  REAL op_tol_rel_gap;  // -T#, relative tolerance (io_diagonal) for distinct vertices (1e-3).
  REAL op_tol_minangle; // -T,# minimum collinear angle (radian) (default < 0.01 degree)
  REAL op_tol_cosminangle; // = cos(op_tol_minangle)
  REAL op_tol_cosmaxangle; // = cos(PI - op_tol_minangle)
  REAL op_tol_voronoi_minangle; // -T,#,# minimum collinear angle (radian) (default < 0.001 degree)
  ...
  int  op_use_refine_list; // -r
  REAL op_elem_growth_ratio;  // Growth ratio of # elements, -r#, 0.3.
  int  ct_elem_limit; // = tr_tris->objects * (1 + op_elem_growth_ratio);
  ...
  int ct_unused_vrts;
  int ct_segments, ct_unused_segs;
  ...
  int  insert_vertices(Vertex **ptlist, int ptnum, int hullflag);
  int  incremental_delaunay();

  // Constrained Delaunay triangulation (constrained.cpp)
  bool is_fixed_vertex(Vertex *pt, Vertex **adj1, Vertex **adj2);
  ...
  int  recover_segments(bool save_flag = false);
  ...
  int  get_cct_location(TriEdge &T, bool hull_flag);
  ...
  int  get_powercell(Vertex *mesh_vertex, bool calc_cct_flag, Vertex** pptlist, int* ptnum);
  int  get_constrained_power_cell(Vertex *mesh_vertex, TriEdge &Estart, TriEdge &Eend, Vertex** pptlist, int* ptnum);
