...
//==============================================================================
// Dertermine whetehr the given vertex (an input or a Steiner point) is free
//   to be rmeoved or not.
//
// If the vertex is not fixed (return false), and if "adj1 != NULL" and
//   "adj2 != NULL", they return the two adjacent vertices of "v".
//   They can be used to calculate the Laplacian center.

bool Triangulation::is_fixed_vertex(Vertex *v, Vertex **adj1, Vertex **adj2)
{
  // Count how many segments at this vertex (from the link list).
  int j = 0;
  TriEdge seg = v->on_seg;
  while (seg.tri != NULL) {
    j++;
    // The vertex-to-segment map can be validated by check_mesh().
    //if (!(seg.tri->vrt[seg.ver] == v)) {
    //  quit(QUIT_ON_BUG);
    //}
  ...
  if (j == 0) {
    ...
    // There are more than 3 segments at this vertex.
    return true;  // It is a ridge vertex.
  }

  // j = 2, there are exactly two segments at this vertex.
  TriEdge seg1 = v->on_seg;
  ...
  if (seg1.tri->tag != seg2.tri->tag) {
    ...
    REAL cosang = get_cosangle(v, e1, e2);
    //printf(" debug: angle at %d = %.17g degree\n", v->idx, acos(cosang) / PI * 180.0);
    if (cosang > op_tol_cosmaxangle) {
      // They are not (nearly) collinear.
      return true; //v->set_fix(); // v->typ = RIDGEVERTEX;
    } else {
      // It is not a fixed vertex on a line.
...
int Triangulation::insert_segment(TriEdge E, int stag, REAL val, Triang** pseg)
{
  ...
  for (int j = 0; j < 2; j++) {
    ...
    TriEdge nextseg = seg->vrt[j]->on_seg;
    seg->vrt[j]->on_seg = TriEdge(seg, j);
    seg->vrt[j]->on_seg.tri->nei[j] = nextseg;
...
int Triangulation::insert_segment_intersect(Vertex *e1, Vertex *e2, int stag, REAL val)
{
  ...
  while (startpt != NULL) {
    ...
    if (chk == INTERSECT_NONE) {
      ...
      if (!(result == INTERSECT_SHARE_EDGE)) {
        quit(QUIT_ON_BUG);
      }
      insert_segment(S, stag, val, NULL);
      if (fqueue->objects > 0) {
...
int Triangulation::recover_segments(bool save_flag)
{
  if (op_db_verbose) {
  ...
  for (int i = 0; i < tr_segs->objects; i++) {
    Segment *seg = (Segment *) tr_segs->get(i);
    ...
    if (res == INTERSECT_SHARE_EDGE) {
      if (!E.is_segment()) {
        ...
        for (int j = 0; j < 2; j++) {
          ...
          TriEdge pseg = seg->vrt[j]->on_seg;
          seg->vrt[j]->on_seg = TriEdge(seg, j);
          seg->vrt[j]->on_seg.tri->nei[j] = pseg;
          //// Check if it becomes a ridge vertex (un-removable).
          //if (is_fixed_vertex(seg->vrt[j])) {
          //  seg->vrt[j]->set_fix();
          //} else {
      ...
      if (save_flag) {
        if (op_db_verbose > 1) {
          printf("  !! Segment [%d, %d] is not recovered\n", seg->vrt[0]->idx, seg->vrt[1]->idx);
        }
        seg->set_exterior();
        ct_unused_segs++;
        continue;
  ...
  if (ct_unused_segs > 0) {
    if (op_db_verbose) {
      printf("!! %d segments are not recovered.\n", ct_unused_segs);
    }
    return 0;
  }
...
int Triangulation::set_ridgevertices()
{
  ...
  for (int i = 0; i < ct_in_vrts; i++) {
    ...
    v->clear_fix();
    if (is_fixed_vertex(v, NULL, NULL)) {
      v->set_fix();
      rcount++;
    }
  }
