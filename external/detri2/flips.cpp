...
int Triangulation::flip_check(TriEdge *te)
{
  ...
  if ((s1 != 0.0) && te->enext().is_segment() && tn.eprev().is_segment()) {
    // Avoid creating a very skinny triangle at two (nearly) collinear segments.
    // check if the two segments are nearly collinear.
    // Check the angle at v (between edges [v,e1] and [v,e2])
    REAL cosang = get_cosangle(pb, pc, pd);
  ...
  if ((s2 != 0.0) && te->eprev().is_segment() && tn.enext().is_segment()) {
    ...
    // check if the two segments are nearly collinear.
    // Check the angle at v (between edges [v,e1] and [v,e2])
    REAL cosang = get_cosangle(pa, pc, pd);
    //printf(" debug: angle at %d = %.17g degree\n", v->idx, acos(cosang) / PI * 180.0);
    if (cosang < op_tol_cosmaxangle) {
      // They are nearly collinear.
      s2 = 0.0;
